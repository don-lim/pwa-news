const apiKey = '5cdaeed1c0cb469f94f672b7b8c9be76';
...
async function techNews() {
	console.log('techNews fired');
  newsArticles.innerHTML = '';
  const response = await fetch(`https://newsapi.org/v2/everything?q=ticketing%20system&apiKey=${apiKey}`);
  const json = await response.json();
  console.log("%j", json);
  newsArticles.innerHTML =
    json.articles.map(createArticle).join('\n');
